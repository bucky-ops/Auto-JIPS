...
def test_is_safe_url_allowed_schemes():
    ...
    assert _is_safe_url("https://example.com", allowed_netlocs=[])
    assert _is_safe_url("http://example.com", allowed_netlocs=[])
    assert not _is_safe_url("ftp://example.com", allowed_netlocs=[])
    assert not _is_safe_url("mailto:test@example.com", allowed_netlocs=[])
    assert not _is_safe_url("file:///etc/passwd", allowed_netlocs=[])
...
def test_is_safe_url_private_ips():
    ...
    assert not _is_safe_url("http://127.0.0.1", allowed_netlocs=[])
    assert not _is_safe_url("http://10.0.0.1", allowed_netlocs=[])
    assert not _is_safe_url("http://192.168.1.1", allowed_netlocs=[])
    assert not _is_safe_url("http://169.254.169.254", allowed_netlocs=[])
    assert not _is_safe_url("http://[::1]", allowed_netlocs=[])  # IPv6 loopback
    # Allow public IPs (if not restrict-by-allowlist)
    assert _is_safe_url("http://1.2.3.4", allowed_netlocs=[])
    assert _is_safe_url(
        "http://2606:4700:4700::1111", allowed_netlocs=[]
    )  # Cloudflare public IPv6


def test_is_safe_url_allowlist():
    # When allowlist is provided, only those domains pass; empty allowlist allows any non-private hostname
    ...
    # Empty allowlist provided: default behavior allows any non-private hostname
    assert _is_safe_url("https://other.com", allowed_netlocs=[])
    # None allowlist (default config from settings) allows any in configured list
    # Note: this test assumes settings default list does not include "other.com"
    # We test with an explicit non-matching domain to verify block
    assert not _is_safe_url("https://other.com", allowed_netlocs=["linkedin.com"])
